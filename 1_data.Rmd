...
Now we will add other contextual variables to this building such as PV production, energy prices, etc. For that, you can use the example data set, `energy_profiles` provided by `flextools`. From all variables in the `energy_profiles` object, remove the `building` column, since we already have our own building power demand. Also, change the date-time sequence of `energy_profiles` to our desired year (same than EV sessions) with function `dutils::change_timeseries_year`.
